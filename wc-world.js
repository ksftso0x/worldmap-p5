const ratio = 0.75;
let ww = 1000; // window.innerWidth;
let hh = 750; // window.innerHeight; // ww * ratio;
var layerCnt = 5;
var stepsLayer = 50;
var pp = [[-200, -100], [200, -100], [0, -200], [-150, 200], [200, 200]];
...
function preload() {
    imgMask = loadImage('assets/world.png');
}
...
function draw() {
  ...
  for (var fcn = 0; fcn < layerCnt - 2; fcn++) {
  ...
  for (fcn = layerCnt - 2; fcn < layerCnt; fcn++) {
      fill_colors[fcn] = new Object;
      fill_colors[fcn].rr = random(1, 255);
      fill_colors[fcn].gg = random(1, 255);
      fill_colors[fcn].bb = random(1, 255);
  ...
  for (fcn = 0; fcn < layerCnt; fcn++) {
      myLayers[fcn] = new Array(stepsLayer).fill().map((x, i) => deform(poly(random(300, 400), noise(i) * 30), 8, 0.5));
      layerLen += stepsLayer;
  }
