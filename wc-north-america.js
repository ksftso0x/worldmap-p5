const ratio = 0.83;
let ww = 820; // window.innerWidth;
let hh = 680; // window.innerHeight; // ww * ratio;
var layerCnt = 5;
var stepsLayer = 50;
var pp = [[-200, -150], [100, -150], [0, -100], [-100, 100], [150, 100]];
...
function preload() {
    imgMask = loadImage('assets/north-america.png');
}
...
function draw() {
  ...
  for (fcn = 0; fcn < layerCnt; fcn++) {
      myLayers[fcn] = new Array(stepsLayer).fill().map((x, i) => deform(poly(random(150, 250), noise(i) * 30), 8, 0.5));
      layerLen += stepsLayer;
  }
